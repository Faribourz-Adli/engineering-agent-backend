import os, io, re, json, uuid, tempfile, subprocess, glob
...
def _text_useful(t: str) -> bool:
    ...
    return len((t or "").strip()) > 200 and len(_re.findall(r'[A-Za-z]{3,}', t or "")) > 20
...
def _has_signal_terms(t: str) -> bool:
    ...
    return any(_re.search(p, t or "", _re.I) for p in signals)
...
# ---------- METADATA / SECTION / TERM EXTRACTION ----------
def simple_extract_metadata(text: str, file_name: Optional[str] = None):
    ...
    return {"code": code, "title": title or (code or "Untitled Standard"), "publisher": pub or "Company", "rev_date": rev_date}

def segment_sections(text: str):
    lines = [l for l in text.splitlines()]
    sections, current = [], {"path": "0", "heading": "Document", "body": []}
    for ln in lines:
...
def debug_extract(file_path: str, pages: int = 15):
    """
    Download a PDF from 'raw' and show short previews from:
    - pdftotext
    - OCRmyPDF -> pdftotext
    - high-DPI image OCR
    """
    ...
    try:
        with tempfile.TemporaryDirectory() as td:
            ...
            with open(ip,"wb") as f: f.write(content)
            subprocess.run(["pdftotext","-layout","-f","1","-l",str(pages),ip,out], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            txt = open(out,"r",encoding="utf-8",errors="ignore").read() if os.path.exists(out) else ""
            result["pdftotext"] = _preview(txt)
    ...
    try:
        with tempfile.TemporaryDirectory() as td:
            ...
            with open(ip,"wb") as f: f.write(content)
            subprocess.run(["ocrmypdf","--skip-text","--force-ocr","--rotate-pages","--deskew","--output-type","pdf",ip,op], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            subprocess.run(["pdftotext","-layout","-f","1","-l",str(pages),op,out], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            txt = open(out,"r",encoding="utf-8",errors="ignore").read() if os.path.exists(out) else ""
            result["ocrmypdf_pdftotext"] = _preview(txt)
    except Exception as e:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with open(ip,"wb") as f: f.write(content)
            subprocess.run(["pdftoppm","-r","300","-f","1","-l","10","-png",ip,base], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            chunks = []
            for img_path in sorted(glob.glob(base + "-*.png")):
                img = Image.open(img_path)
                chunks.append(pytesseract.image_to_string(img, config="--oem 1 --psm 6 -l eng"))
            txt = "\n".join(chunks)
            result["image_ocr"] = _preview(txt)
    except Exception as e:
...
def enqueue(req: EnqueueProcessReq):
    ...
    try:
        ...
        std_id = upsert_standard_and_children(meta, text, req.upload_session_id, os.path.basename(req.file_path))
        # store raw text + meta JSON
        ...
        sb.table("processing_jobs").update({"status": "failed", "finished_at": datetime.utcnow().isoformat(), "log": str(e.detail)}).eq("upload_session_id", req.upload_session_id).execute()
        return JSONResponse(status_code=400, content={"error": str(e.detail)})
    except Exception as e:
        sb.table("processing_jobs").update({"status": "failed", "finished_at": datetime.utcnow().isoformat(), "log": str(e)}).eq("upload_session_id", req.upload_session_id).execute()
...
def list_xrefs(standard_id: str):
    ...
    for l in links:
        ...
        if to:
            items.append({"xref_id": l["id"], "to_standard": to, "evidence_text": l.get("evidence_text"), "confidence": l.get("confidence", 0.0)})
...
def conflicts_csv(standard_id: str):
    ...
    return Response(content=csv_data, media_type="text/csv", headers={"Content-Disposition": f'attachment; filename="conflicts_{standard_id}.csv"'})
